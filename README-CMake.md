...
- [Build `simh` using CMake](#build-simh-using-cmake)
  ...
  - [Building the simulators](#building-the-simulators)
    ...
    - [CMake Command Line](#cmake-command-line)
      ...
      - [Linux/Unix/Unix-like/MacOSX/MinGW-w64 walkthrough](#linuxunixunix-likemacosxmingw-w64-walkthrough)
    ...
    - [Visual Studio-Only Development](#visual-studio-only-development)
      ...
      - [XP-compatible Build via the VS2019 IDE](#xp-compatible-build-via-the-vs2019-ide)
...
- [CMake][cmake] version 3.14 or newer.
  ...
  - MacOSX: Install `cmake` using your preferred external package management
...
- The [Git source control system][gitscm].
  ...
  - MacOSX: Install `git` using your preferred external package management
    system, such as [Homebrew][homebrew] or [MacPorts][macports]
...
- GNU Make: 
  ...
  - Required for Linux and MacOSX. Consult your appropriate package manager to
...
_VS2022_: Building XP-compatible and Server 2003 binary compatible binaries is no longer available.

_VS2019_: Install the `v141` (VS 2017) tools.

- Start up and open the Visual Studio Installer.
...
- Click on the checkbox next to "C++ Windows XP Support for VS 2017 (v141) tools [Deprecated]" item.
...
the [command line](#cmake-command-line) or [via the IDE](#visual-studio-only-development).
...
- `CMake` version 3.14 or higher. The `CMake` distributed with VS 2017 is
  ...
  - You will need to install a newer version of `CMake` (see above
    for download links.) 
  - You will also need to ensure that the directory to updated `cmake.exe` is on
...
TUN/TAP header file on Linux and MacOSX, and VDE is an optionally installed
package on Linux and MacOSX.

#### Linux, MacOSX and MinGW-w64
...
and MacOSX. `.travis/deps.sh` can also install the requisite toolchains and
...
- Linux/Unix-lib/MacOSX/MinGW-w64:
...
- Linux/Unix-lib/MacOSX/MinGW-w64:
    ...
    --nolto           Disable Link Time Optimization (LTO) in Release builds
...
- Windows PowerShell:
    ...
    # List the supported command line flags:
    PS C:\...\open-simh> cmake\cmake-builder.ps1 -help
    ...
    -nolto                 Disable Link Time Optimization in Release builds
...
#### Linux/Unix/Unix-like/MacOSX/MinGW-w64 walkthrough

The following walkthrough shows how to manually configure, build, test and
install the simulators from a Linux and MacOSX terminal window, and a MinGW-w64
...
Windows follows the same pattern as the Linux/MacOSX/MinGW-w64 walkthrough with
...
# command line -- you must have VS 2017 or VS 2019 AND the v141_xp toolkit
# installed.
...
| `NO_DEP_BUILD`       | platform-specific  | Build dependency libraries on Windows (def: enabled), disabled on Linux/MacOSX, MinGW-w64. |
| `BUILD_SHARED_DEPS`  | platform-specific  | Build dependencies as shared libraries/DLLs on Windows. Does nothing on Linux/MacOSX. Disabled by default on Windows to ensure that the simulators link against static libraries. |
...
| `DONT_USE_ROMS`      | disabled           | Build support ROMs. |
...
- Linux/MacOSX/MinGW-w64 example:
...
approach. If the `cppcheck` executable is detected on your `PATH`, you will be
able to use the `cppcheck` rule to run the checker over all of SIMH's source:

```sh
## Execute from inside your build directory.
## (note: the command is the same for Windows)
$ cbuild --build . --target cppcheck
``` 
...
#### XP-compatible Build via the VS2019 IDE
...
- Add the `x86-Debug` configuration, similar to step 4, with the following
  ...
  - Change the "CMake generator" to "Visual Studio 16 2019" from the "CMake
    generator" dropdown. 
...
$ python3 -m generate
generate.py: Looking for makefile, starting in [simh_source]/cmake
generate.py: Looking for makefile, trying [simh_source]
generate.py: Processing [simh_source]/makefile
...
==== writing to [simh_source]/3B2/CMakeLists.txt
==== writing to [simh_source]/ALTAIR/CMakeLists.txt
==== writing to [simh_source]/AltairZ80/CMakeLists.txt
==== writing to [simh_source]/B5500/CMakeLists.txt
==== writing to [simh_source]/BESM6/CMakeLists.txt
==== writing to [simh_source]/CDC1700/CMakeLists.txt
==== writing to [simh_source]/GRI/CMakeLists.txt
==== writing to [simh_source]/H316/CMakeLists.txt
==== writing to [simh_source]/HP2100/CMakeLists.txt
==== writing to [simh_source]/HP3000/CMakeLists.txt
==== writing to [simh_source]/I1401/CMakeLists.txt
==== writing to [simh_source]/I1620/CMakeLists.txt
==== writing to [simh_source]/I650/CMakeLists.txt
==== writing to [simh_source]/I7000/CMakeLists.txt
==== writing to [simh_source]/I7094/CMakeLists.txt
==== writing to [simh_source]/Ibm1130/CMakeLists.txt
==== writing to [simh_source]/Intel-Systems/Intel-MDS/CMakeLists.txt
==== writing to [simh_source]/Intel-Systems/scelbi/CMakeLists.txt
==== writing to [simh_source]/Interdata/CMakeLists.txt
==== writing to [simh_source]/LGP/CMakeLists.txt
==== writing to [simh_source]/NOVA/CMakeLists.txt
==== writing to [simh_source]/PDP1/CMakeLists.txt
==== writing to [simh_source]/PDP10/CMakeLists.txt
==== writing to [simh_source]/PDP11/CMakeLists.txt
==== writing to [simh_source]/PDP18B/CMakeLists.txt
==== writing to [simh_source]/PDP8/CMakeLists.txt
==== writing to [simh_source]/PDQ-3/CMakeLists.txt
==== writing to [simh_source]/S3/CMakeLists.txt
==== writing to [simh_source]/SAGE/CMakeLists.txt
==== writing to [simh_source]/SDS/CMakeLists.txt
==== writing to [simh_source]/SEL32/CMakeLists.txt
==== writing to [simh_source]/SSEM/CMakeLists.txt
==== writing to [simh_source]/TX-0/CMakeLists.txt
==== writing to [simh_source]/VAX/CMakeLists.txt
==== writing to [simh_source]/alpha/CMakeLists.txt
==== writing to [simh_source]/imlac/CMakeLists.txt
==== writing to [simh_source]/sigma/CMakeLists.txt
==== writing to [simh_source]/tt2500/CMakeLists.txt
==== writing [simh_source]/cmake/simh-simulators.cmake
```
