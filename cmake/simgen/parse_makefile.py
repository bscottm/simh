...
_variable_rx = re.compile(r"\s*([A-Za-z]\w+)\s*=\s*(.*)")
_rule_rx     = re.compile(r"(((\$[({])*\w[\w-]+[)}]*)+)\s*:\s*(.*)")

# Regex that recognizes variables. Group 1 is the variable's name.
_var_rx      = re.compile(r"^\$[{(]([A-Za-z]\w*)[)}]$")
_var_rx2     = re.compile(r"\$[{(]([A-Za-z]\w*)[)}]")
_norm_var_rx = re.compile(r"\$\(([A-Za-z]\w*)\)")
