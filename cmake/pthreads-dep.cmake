...
# (b) MSVC: Build Pthreads4w as a dependent unless using vcpkg.
#~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=

add_library(thread_lib INTERFACE)
set(AIO_FLAGS)

## Both async I/O and SLiRP use pthreads.
if (NOT WITH_ASYNC)
